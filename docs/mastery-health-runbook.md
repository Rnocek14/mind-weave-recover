...
- `telemetry_write_failures_24h` reads from `adaptation_anomalies` but the writer
  in `useAdaptationTrialLogger` currently inserts into `exercise_events` with a
  non-existent column. Metric will read 0 until that writer is repointed.
