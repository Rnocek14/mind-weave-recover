...
/**
 * Generate accessible exercise recommendations based on capability scores
 */
export function getAccessibleExercises(scores: CapabilityScores): string[] {
  const exercises: string[] = [];
  
  // Level 0-2 exercises (pure sensorimotor)
  if (scores.vision >= 3 && scores.motor >= 3) {
    exercises.push('reach-tap');
  }
  
  // Level 3-4 exercises (pattern matching)
  if (scores.vision >= 5 && scores.motor >= 4 && scores.attention >= 4) {
    exercises.push('photo-naming-simple'); // Simplified 2-choice version
  }
  
  // Level 5+ exercises (full cognitive load)
  if (scores.vision >= 7 && scores.motor >= 6 && scores.attention >= 6) {
    exercises.push('photo-naming', 'phonological', 'semantic-feature');
  }
...
/**
 * Generate required adaptations based on capability profile
 */
export function getRequiredAdaptations(scores: CapabilityScores): {
