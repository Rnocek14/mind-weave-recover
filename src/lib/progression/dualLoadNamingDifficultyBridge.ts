/**
 * Dual-Load Naming — Clinical Level → Engine Difficulty bridge.
 *
 * The dual-load-naming engine uses the universal 1–10 difficulty tier
 * (its `levelToTierLocal` collapses to bank tiers 1–3). This bridge
 ...
 *
 * Mapping rationale (see docs/clinical-evidence/dual-load-naming.md):
 *   Clinical L1 → tier 1   (familiar, light bar)
 *   Clinical L2 → tier 2   (familiar, full coverage)
 *   Clinical L3 → tier 3   (familiar extended)
 *   Clinical L4 → tier 4   (familiar extended + recall integrity)
 *   Clinical L5 → tier 6   (mixed-frequency)
 *   Clinical L6 → tier 7   (mixed-frequency sustained)
 *   Clinical L7 → tier 8   (low-frequency — thin bank)
 ...
 * SentenceConstruction/MultiStepPlanning/SynonymGenerator.
 *
...
export function resolveEffectiveDualLoadNamingInitialDifficulty(args: {
