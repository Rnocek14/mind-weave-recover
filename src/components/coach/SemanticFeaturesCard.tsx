...
interface SemanticFeaturesCardProps {
  ...
  conversationContext?: string; // Last few words from conversation
}
...
export function SemanticFeaturesCard({ difficulty, onComplete, conversationContext }: SemanticFeaturesCardProps) {
  ...
  // Auto-start listening when trial is ready
  useEffect(() => {
    if (trial && phase === 'listening' && !isListening) {
      startListening();
    }
  }, [trial, phase, isListening, startListening]);

  // Auto-complete after 3 seconds of speech or transcript
  useEffect(() => {
    if (transcript.trim().length >= 3 && phase === 'listening') {
      const timer = setTimeout(() => {
        handleDone();
      }, 2000); // 2s after they start speaking
  ...
  }, [stopListening, transcript, trial, onComplete]);
