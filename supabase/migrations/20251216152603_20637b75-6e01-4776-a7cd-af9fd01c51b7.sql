-- Fix 1: Update claim_speech_analysis_jobs with retry cap in stale reclaim path
...
BEGIN
  ...
  WITH claimed AS (
    ...
    WHERE (
      ...
      -- OR stale processing jobs (stuck > 10 minutes) BUT only if under retry cap
      OR (ua.analysis_status = 'processing' AND ua.locked_at < now() - interval '10 minutes' AND ua.retry_count < 5)
    )
  ...
  SET 
    ...
    next_retry_at = NULL,  -- Reset retry delay on reclaim
    -- Increment retry count only if reclaiming stale job
...
-- Fix 2: Update submit_speech_analysis_result with correct ROW_COUNT type and backoff math
...
AS $function$
DECLARE
  v_rowcount integer;  -- Fixed: was BOOLEAN, ROW_COUNT returns integer
...
BEGIN
  ...
  ELSIF p_success AND NOT v_has_payload THEN
    ...
    SET 
      ...
      -- Fixed: use (retry_count + 1) for backoff since expression uses pre-update value
  ...
  ELSE
    ...
    UPDATE utterance_analyses
    SET 
      analysis_status = CASE WHEN retry_count >= 4 THEN 'failed' ELSE 'pending' END,  -- Fixed: check >= 4 since we're about to increment
      error_message = p_error_message,
      retry_count = retry_count + 1,
      next_retry_at = CASE 
        WHEN retry_count >= 4 THEN NULL  -- No more retries
        ELSE now() + (interval '1 minute' * power(2, LEAST(retry_count + 1, 6)))  -- Fixed: use retry_count + 1
      END,
      locked_at = NULL,
      locked_by = NULL,
      updated_at = now()
...
-- Fix 3: Add RLS policy for clinicians to view worker heartbeats
CREATE POLICY "Clinicians can view worker heartbeats"
